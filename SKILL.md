...
- If `retCode=0`: credentials are valid. Tell the user:
  ...
  ✓ Connected to Bybit [Mainnet/Testnet].
    Signing: <HMAC-SHA256 | RSA-SHA256 (key: <basename>, <bits>-bit)>
    Account: UNIFIED
...
- If `retCode=10003/10004`: signature error. Append `(current sign type: HMAC|RSA)` to the error message so the user knows which branch ran.
  - Common causes for HMAC: clock drift, wrong secret, non-compact JSON body.
  - Common causes for RSA: (a) private key at `$BYBIT_API_PRIVATE_KEY_PATH` doesn't match the public key uploaded to Bybit; (b) missing `X-BAPI-SIGN-TYPE: 2`; (c) signature output was hex instead of base64; (d) padding is PSS instead of PKCS#1 v1.5.
...
**POST — RSA (place spot market order):**
