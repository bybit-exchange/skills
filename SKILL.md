...
metadata:
  version: 1.6.0  # Modular Architecture + Security Baseline
  author: Bybit
  updated: 2026-08-20
license: MIT
---
...
3. MANIFEST = curl -sf -H "User-Agent: bybit-skill/1.6.0" https://api.bybit.com/skill/manifest
...
6. Version comparison (semver): split by ".", compare major → minor → patch numerically.
   ...
   a. For each file in manifest.files:
      - Download: curl -sf -H "User-Agent: bybit-skill/1.6.0" https://raw.githubusercontent.com/bybit-exchange/skills/main/<file>
...
2. If the module has NOT been loaded in this session:
   a. Ensure manifest is available:
      ...
      - Otherwise: MANIFEST = curl -sf -H "User-Agent: bybit-skill/1.6.0" https://api.bybit.com/skill/manifest
   ...
   b. Download: curl -sf -H "User-Agent: bybit-skill/1.6.0" https://raw.githubusercontent.com/bybit-exchange/skills/main/modules/<module>.md
...
| grid bot, DCA bot, martingale, combo bot, trading bot, create bot, close bot, backtest, 回测, historical performance, bot backtest | **trading-bot** | `modules/trading-bot.md` | account, derivatives |
| TradFi Combo, MT5 combo, tradfi bot, stock portfolio bot, commodity portfolio bot, rebalance bot, XAUUSD+, XAGUSD, USOUSD, 股票组合机器人, 大宗商品组合, 传统金融机器人, 组合调仓机器人 | **tradfi-bot** | `modules/tradfi-bot.md` | account, trading-bot |
...
| xStocks, tokenized stock, commodity perpetual, XAUUSDT, XAGUSDT, CLUSDT, crude oil, buy/sell TradFi, metals agreement, oil agreement | **tradfi** | `modules/tradfi.md` | account, spot, derivatives |
...
- **⚠️ `tradfi` vs `tradfi-bot` (easy to confuse)**: `tradfi` = trading TradFi assets **yourself** (tokenized stocks `TSLAXUSDT`, commodity perps `XAUUSDT`/`CLUSDT`) via standard V5 order endpoints. `tradfi-bot` = the **TradFi Combo auto-rebalancing portfolio bot** over MT5 instruments (`XAUUSD+`, `XAGUSD`, `USOUSD`) via `/v5/mt5combobot/*`. Different symbols, different endpoints. "Buy some gold" → `tradfi`. "Build me a gold+oil portfolio bot" → `tradfi-bot`. If genuinely ambiguous, ask which one.
...
- Common Chinese synonyms: 查价/看价 → market, 买/卖/现货 → spot, 开多/开空/合约/杠杆 → derivatives, 理财/质押/双币/持币生息/私人财富 → earn, 余额/转账/充值/提币 → account, 跟单 → copy-trading, 网格/DCA/AI推荐/一键创建/策略推荐/回测/历史表现 → trading-bot, 组合机器人/调仓机器人/股票组合/大宗商品组合/传统金融机器人 → tradfi-bot, 链上/meme/DEX/代币/预测/押注/预测市场/世界杯/FIFA → alpha-trade, 买入代币化股票/特斯拉/苹果/英伟达/黄金/白银/原油/商品永续 → tradfi, 拆单/算法单/POV → strategy, 银行卡/消费记录/刷卡 → card, 打新/新币挖矿/launchpool/拼图/代币空投/活动列表/质押活动 → activity, 授权/登录/连接Bybit/OAuth → oauth
...
curl -s "${BASE_URL}/v5/position/list?${QUERY}" \
  ...
  -H "User-Agent: bybit-skill/1.6.0" \
...
| execType | `Trade` `AdlTrade` `Funding` `BustTrade` `Delivery` `Settle` `BlockTrade` `MovePosition` `FutureSpread` `SessionSettlePnL` `CoinSwap` `CorporateAction` `ForwardSplitSettle` `ReverseSplitSettle` |
