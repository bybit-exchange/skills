...
## Aurora AI Params Recommendation
...
> **Testnet caveat.** Aurora is driven by live production data; testnet coverage is unverified. If an Aurora endpoint returns `404` / empty recommendations / permission errors on `BYBIT_ENV=testnet`, don't retry — tell the user Aurora may not be available on testnet and suggest switching to mainnet. Do NOT preemptively block the call.
...
|----------|------|--------|-----------------|-----------------|------------|
| Home Recommendations | `/v5/aurora/home` | POST | — | — | Aurora |
| Creation-Page Recommendations | `/v5/aurora/creation` | POST | biz_type, symbol | — | Aurora |
| Explore-Page Recommendations | `/v5/aurora/explore` | POST | biz_type | — | Aurora |
| EasyBot Recommendation | `/v5/aurora/easy` | POST | symbol, product, direction | — | Aurora |
| Get Strategy by ID | `/v5/aurora/info` | POST | aurora_id | — | Aurora |

### Request Enums

- `biz_type` / `biz` (Business): `0` BIZ_UNKNOWN | `1` SPOT_GRID | `2` FUTURE_GRID | `3` SPOT_DCA | `4` DUAL_ASSET | `5` LIQUIDITY_MINING | `6` COPY_TRADING | `7` FUTURE_MARTIN | `8` FUTURE_COMBO
- `product` (ProductType): `0` PRODUCT_TYPE_UNKNOWN | `1` PRODUCT_TYPE_SPOT | `2` PRODUCT_TYPE_FUTURE
- `direction` / `market_mode` / `grid_mode` (GridMode): `0` GRID_MODE_UNSPECIFIED | `1` GRID_MODE_NEUTRAL | `2` GRID_MODE_LONG | `3` GRID_MODE_SHORT
- `style` (BotStyle): `0` STYLE_UNSPECIFIED | `1` STYLE_HIGH_PROFITS | `2` STYLE_STEADY | `3` STYLE_HIGH_FREQUENCY

### Endpoint Notes

**`/v5/aurora/home`** — Home feed. Empty request body (`{}`) — recommendations are personalized server-side. Returns up to **18 strategies** mixed across bot types; only **6** are returned for Copy Trading leaders. Inspect each item's `biz_type` to know which create endpoint applies.

**`/v5/aurora/creation`** — Creation-page recommendations for a known bot type and symbol. Returns up to **6 strategies** plus a top-level `market_mode` (Aurora's suggested direction for this symbol) that can be used to pre-select grid direction in the UI.

**`/v5/aurora/explore`** — Explore-page recommendations for a known bot type only (symbol not yet chosen). Returns up to **6 strategies** spanning multiple symbols. Once the user picks a symbol, switch to `/v5/aurora/creation` to narrow down.

**`/v5/aurora/easy`** — One-click EasyBot flow. User provides `symbol` + `product` (spot/futures) + `direction`; Aurora picks the bot type and returns a single strategy. Response includes top-level `biz` — use it to route to the correct create endpoint (e.g. `biz=1` → spot grid create, `biz=2` → futures grid create).

**`/v5/aurora/info`** — Refetch a previously recommended strategy by its `aurora_id` (which came from any of the 4 recommendation endpoints above). Useful for refreshing backtest metrics or re-displaying params.

### Response Payload — AIParamsStrategy

Each recommendation returns an `AIParamsStrategy` object. **Exactly one** of `spot_bot` / `fgrid_bot` / `fmartin_bot` / `fcombo_bot` is populated, determined by `biz_type`:

- `biz_type=1` SPOT_GRID → `spot_bot` = `{upper_price, lower_price, grid_num}`
- `biz_type=2` FUTURE_GRID → `fgrid_bot` = `{lower_price, upper_price, grid_num, leverage_e2}`
- `biz_type=7` FUTURE_MARTIN → `fmartin_bot` = `{profit_target_rate, open_limit_rate, max_open_count, open_amount_multipler, leverage_e2}`
- `biz_type=8` FUTURE_COMBO → `fcombo_bot` = `{tokens[], leverage, resize_time, resize_ratio_e2}` where each `tokens[]` entry is `{base_coin, quote_coin, symbol, ratio_e2, mode}` and `resize_time` is in minutes

Backtest metrics on every strategy: `estimate_profits_days`, `estimate_profits_rate_e4`, `estimate_profits_apr_e4`, `yield_trend_list[]` (14-day yield curve of `{stat_time, yield_rate_e4}`), `max_draw_down_rate_e4`, `atr_e4`, `sell_cnt` (backtest arbitrage count), `sharpe_ratio_e4`.

> **Scaling convention**: fields suffixed `_e2` are ×100 (divide by 100 for the real value, e.g. `leverage_e2=500` → 5x leverage, `ratio_e2=20` → 20%); fields suffixed `_e4` are ×10000 (divide by 10000, e.g. `yield_rate_e4=50` → 0.5%).

> **`aurora_id`** on every strategy is an opaque encoded ID — pass it to `/v5/aurora/info` to refetch the full strategy later.
