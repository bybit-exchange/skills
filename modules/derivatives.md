...
* **execType**: `Trade` | `AdlTrade` | `Funding` | `BustTrade` | `Delivery` | `Settle` | `BlockTrade` | `MovePosition` | `FutureSpread` | `SessionSettlePnL` | `CoinSwap` | `CorporateAction` | `ForwardSplitSettle` | `ReverseSplitSettle`
  - `FutureSpread`: spread leg execution (see the Spread Trading section in `modules/advanced.md`).
  - `ForwardSplitSettle` / `ReverseSplitSettle`: fractional-share settlement generated by a forward / reverse stock split (tokenized stocks). Appears in `/v5/execution/list` and `/v5/position/closed-pnl`; also accepted as an `execType` filter value.
